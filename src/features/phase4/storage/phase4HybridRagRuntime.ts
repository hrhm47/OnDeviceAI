...
import {
  ...
  PHASE4_HYBRID_RAG_DB_NAME
} from "./phase4HybridRagDb";

export type Phase4EmbeddingMode = "skip" | "ifReady" | "requireReady";
export type Phase4SeedTableCounts = Awaited<
  ReturnType<typeof getPhase4SeedTableCounts>
>;
...
export type Phase4HybridRagRuntime = {
  ...
  status: {
    ...
    userId: string;
    projectId: string;
    seedTableCounts: Phase4SeedTableCounts;
...
export const preparePhase4HybridRagRuntime = async (input?: {
  ...
  const runtimeKey = [
    ...
    `dataset-v${PHASE4_DATASET_VERSION}`,
    PHASE4_HYBRID_RAG_DB_NAME,
    "retrieval-items-v2.1",
    embeddingMode,
  ...
  const seedTableCounts = await getPhase4SeedTableCounts(db);
  ...
  cachedRuntime = {
    ...
    status: {
      ...
      databaseName: PHASE4_HYBRID_RAG_DB_NAME,
      ...
      message: [
        // fts?.supported
          // ? `Retrieval ready for ${contextResult.context.project.project_name} using dataset v${PHASE4_DATASET_VERSION}.`
